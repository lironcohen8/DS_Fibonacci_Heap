...
public class FiboHeapTest {
    ...
    public static int getNumMarked(FibonacciHeap heap) {
        return (heap.potential() - heap.treeNum) / 2;
    }
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.treeNum, numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.treeNum != numOfTrees + heap2.treeNum) {
    ...
    /*public static void testKMin(FibonacciHeap heap) {
        if (heap.size() == 0) {
            ...
            FibonacciHeap heaps[] = new FibonacciHeap[heap.treeNum];
            int heapIndex = 0;
            do {
                next = cur.getNextBro();
                cur.setPrevBro(cur);
                ...
                heapIndex++;
                cur = next;
            } while (next != null && next != heap.first);
...
*/
    public static void testRootPointers(FibonacciHeap heap) {
        ...
        do {
            ...
            if (cur.getNextBro().getPrevBro() != cur)
                throw new RuntimeException("getNext.getPrevBro did not return current root: " + cur.getKey());
            if (cur.getPrevBro().getNextBro() != cur)
                throw new RuntimeException("getPrevBro.getNext did not return current root: " + cur.getKey());
    ...
    public static void testRanks(FibonacciHeap.HeapNode treeRoot) {
        ...
        FibonacciHeap.HeapNode child = treeRoot.getFirstChild();
        ...
        if (rank == 0) {
            ...
            do {
                childrenAmount++;
                if (curNode.getFirstChild() != null) testRanks(curNode.getFirstChild());
                curNode = curNode.getNextBro();
            } while (curNode != child);
    ...
    public static int calculateNumOfTrees(FibonacciHeap heap) {
        ...
        FibonacciHeap.HeapNode cur = heap.first.getNextBro();
        while (cur != heap.first) {
            i++;
            cur = cur.getNextBro();
        }
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.treeNum) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.treeNum, actualNumOfTrees));
        }
    }
    ...
    public static void testSiblings(FibonacciHeap.HeapNode root) {
        if (root.getFirstChild() != null) {
            FibonacciHeap.HeapNode child = root.getFirstChild();
            FibonacciHeap.HeapNode cur = child;
            do {
                if (cur.getPrevBro().getNextBro() != cur)
                    throw new RuntimeException("getPrevBro.getNext did not return current child: " + cur.getKey());
                if (cur.getNextBro().getPrevBro() != cur)
                    throw new RuntimeException("getNext.getPrevBro did not return current child: " + cur.getKey());
                cur = cur.getNextBro();
            } while (cur != child);
        }
    }
    ...
    public static void testChildren(FibonacciHeap.HeapNode node) {
        if (node.getFirstChild() != null) {
            if (node.getFirstChild().getParent() != node)
                throw new RuntimeException("getChild.getParent did not return cur node: " + node.getKey());
            FibonacciHeap.HeapNode child = node.getFirstChild();
            testChildren(child);
            FibonacciHeap.HeapNode cur = child;
            do {
    ...
    public static void testHeapMinProp(FibonacciHeap.HeapNode node) {
        if (node.getFirstChild() != null) {
            FibonacciHeap.HeapNode firstChild = node.getFirstChild();
    ...
    private static int countTreeItemsRec(FibonacciHeap.HeapNode child) {
        ...
        do {
            cnt++;
            if (cur.getFirstChild() != null) cnt += countTreeItemsRec(cur.getFirstChild());
            cur = cur.getNextBro();
        } while (cur != child);
        return cnt;
    ...
    public static int countTreeItems(FibonacciHeap.HeapNode root) {
        return root == null ? 0 : 1 + countTreeItemsRec(root.getFirstChild());

    }

    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.treeNum) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
                            heap.treeNum, expected.numOfTrees)
            );
        }
